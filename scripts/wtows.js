...
function setInputFields()
  {
	...
	drawPerMonth("permonth",adMonthHeating,adMonthCooling,
	 ...
	 var input = document.getElementsByName("windowSouthP")[0];
	 input.value = dWindowSouthP.toFixed(2);
...
Initializes wtows.xhtml
 ---------------------------------------------------------------------*/
  function onLoad()
  {
    ...
    var select = document.getElementsByName("windowSouthP")[0];
    select.focus();  
  }; /* onLoad */
...
{
  var bValid = false;
  var dPercentage = percentNumber("windowSouthP",MESSAGES.percentage_numeric,MESSAGES.percentage_bounds);
  if (dPercentage != null)
    bValid = true;
  return bValid;
}; /* validate */
